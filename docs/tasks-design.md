...
- **References** — each direction can be turned off entirely via `taskMigrationAddToReference` / `taskMigrationAddFromReference` (both default on); when off, that side gets no link (the origin is just re-stamped, the copy is a plain task). When on, the marker is a **global-only** setting driven by a style (`taskMigrationReferenceStyle`: `text` / `emoji`) that reseeds the editable `taskMigrationToMarker` / `taskMigrationFromMarker` from `MIGRATION_REFERENCE_PRESETS`. Default style is `text` with the arrow glyphs `→` / `←` (plain characters, render everywhere); `emoji` (preset `➡️` / `⬅️`) adds a "Pick…" button per marker that opens the shared `EmojiPickerModal` (same grid as the task-status icon editor). `formatReference` emits the bare `[[link]]` when a marker is empty.
