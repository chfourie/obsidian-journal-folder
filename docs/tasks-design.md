...
- **Left click** the status icon → `model.nextStatus(current)`, written via `vault.process`. **Exception:** a status whose `next` points at *itself* (`model.opensPickerOnClick(id)`) opens the status picker panel instead of writing a no-op cycle. The global **`taskClickOpensPicker`** setting (off by default, *Left-click opens the status menu* in the Tasks tab) makes `opensPickerOnClick` return true for **every** status, so left-click opens the picker everywhere — the flag is threaded into `buildTaskModel` (and baked into `model.id`) so all four surfaces inherit it from the one model seam.
...
- **Lucide markers (live preview)** — `migrationReferenceLivePreviewExtension` (`migration-reference-live-preview.ts`) is the editing-view counterpart: a CodeMirror `ViewPlugin` that **replaces** each `lucide:<name>` marker token with its icon (a `Decoration.replace` + `MigrationMarkerWidget`, reusing the `.jf-migration-ref` / `.jf-migration-ref-icon` styling so it fades to the same opacity, full on hover). The replacement is **revealed back to raw text while the selection touches it** so the token stays editable, and the whole extension gates on `editorLivePreviewField` — **inert in Source mode** (raw editing shows the literal token). Only `lucide:` markers are substituted; plain text / emoji markers already read fine as literal characters, so they're left alone. `findLucideMarkerSpans` (pure, unit-tested) locates a standalone `lucide:` token immediately preceding a `[[wikilink]]`; non-reference tokens (no trailing link, no whitespace boundary) are skipped.
...
- **Placement** — `computeInsertion` (pure) inserts per the `taskMigrationPlacement` setting: `after-last-task` (default) / `heading` (uses `taskMigrationHeading`, created if absent) / `top` / `end`. A created heading uses `taskMigrationHeadingLevel` (1–6, default 2 — the level was hardcoded to 2 before it was configurable); an **existing** heading of that text is never re-levelled, tasks just slot under it. `clampHeadingLevel` floors/clamps the value so a bad front-matter override degrades to 2 rather than emitting a broken heading. Placement + heading + heading level are **folder-honored** (the only `task*` fields that are — they're a per-note layout concern).
