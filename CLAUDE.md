...
The header reads **JOURNAL FOLDER (Dynamic)** / **(Static)** with the bracketed mode tag in `var(--text-accent)`. A single **More...** link to the right of the header opens an Obsidian-native `Menu` carrying every secondary action: mode toggle, *Switch to default folder* (conditional), *Set as default folder* (conditional), *Edit folder configuration* (conditional), *Initialise a new journal folder*. The folder picker beneath the header is also a `<span role="button">` that opens a `Menu` of folders rather than a native `<select>`, so both popups use the same Obsidian floating-panel styling. Mouse triggers route through `Menu.showAtMouseEvent`; keyboard triggers fall back to `Menu.showAtPosition` anchored at the trigger's bounding rect.
