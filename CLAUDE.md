...
- `JournalSignifiersFeature` — renders **signifiers** (tag → icon) in reading view (`processSignifiers` markdown post-processor) and live preview (`signifierLivePreviewExtension`, gated by the `signifierLivePreviewEnabled` kill-switch; the tag is never hidden in live preview), and registers the *Modify signifiers on current line…* command + editor-menu item. Placement is the global `signifierPlacement` setting with four values: `start` (default) / `end` render the icon in normal text flow (`.jf-signifier-lead` / `.jf-signifier-trail` — can't overlap the bullet/checkbox, immune to themes/snippets); `margin` and `margin-column` are opt-in left-margin gutters (`.jf-signifier-gutter` absolutely positioned against the entry's content host / line `.jf-signifier-host`, vertically centred via `top:0;bottom:0`). `margin` hangs the icon just left of **each** entry (indented with nesting); `margin-column` (carries `.jf-signifier-column`) aligns **every** icon in one far-left column (physical-journal left rule). The **horizontal** position of both gutters is set by **measurement, not theme-specific CSS** — a fixed CSS offset collides with themes/snippets that restyle bullets/checkboxes/indentation. Reading view: `gutter-positioner.ts` batches all `getBoundingClientRect` reads then all `left` writes in one `requestAnimationFrame` (one reflow), scheduled after the post-processor and on `workspace.on('css-change')` (theme) + `on('resize')` (column only). Editing view: the live-preview `ViewPlugin` uses CodeMirror's `coordsAtPos` inside a batched `view.requestMeasure` on `docChanged/viewportChanged/geometryChanged`. The offset is scroll-invariant (marker + reference move together) so there is **no scroll recompute**; markers are `visibility:hidden` until the first measurement reveals them (`.jf-positioned`, no wrong-x flash). Per-row recomputes on theme only; the single column also recomputes on resize because its offset includes the possibly width-relative indentation. Warned/layout-dependent opt-in in the settings dropdown (each option lists its pros/cons). An optional `signifierReserveGutter` toggle (global-only, **off by default**, margin modes only) reserves a left lane — sets the content container's `padding-inline-start` to `max(theme padding, widest icon stack + gap)` — so icons never clip with readable line width off / narrow views; it's `max` not an addition (no double margin), the reserve is the intrinsic widest-stack width (no oscillation), and the per-container base padding is `WeakMap`-cached so it never compounds. Signifiers apply to any rendered markdown, not just tasks, and have their **own settings tab**. Signifiers and task categories are **global-only** (in `GLOBAL_ONLY_FIELDS`, never per-folder or per-flow); task categories live under the Tasks tab. See [docs/signifiers.md](docs/signifiers.md).
