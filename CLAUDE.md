...
Desktop picker uses ~1/3 the padding/gap of mobile — `.journal-folder-calendar-picker.is-mobile` overrides restore the roomier mobile spacing. Tap-target dimensions (year-arrow size, month `min-height`, font sizes) stay at their mobile values on both platforms so the panel keeps the same shape; only the air around the elements shrinks on desktop. The user iterated on this — first wanting mobile spacing on desktop, then walking it back to 1/3 — so the current state is "shape is the same, density differs". The options-row's `margin-bottom: 1rem` is halved to `.5rem` via `.journal-folder-header-options:has(+ .journal-folder-calendar)` when the calendar is visible, pulling the controls strip closer to the More-row above without changing the spacing when the calendar is hidden.
