...
- `JournalSignifiersFeature` — renders **signifiers** (tag → icon) in reading view (`processSignifiers` markdown post-processor) and live preview (`signifierLivePreviewExtension`, gated by the `signifierLivePreviewEnabled` kill-switch; the tag is never hidden in live preview), and registers the *Modify signifiers on current line…* command + editor-menu item. Placement is the global `signifierPlacement` setting with four values: `start` / `end` render the icon in normal text flow (`.jf-signifier-lead` / `.jf-signifier-trail` — can't overlap the bullet/checkbox, immune to themes/snippets); `margin` and `margin-column` (the **default**) are left-margin gutters (`.jf-signifier-gutter` absolutely positioned against the entry's content host / line `.jf-signifier-host`, vertically centred via `top:0; bottom:auto; height:1lh`). `margin` hangs the icon just left of **each** entry (indented with nesting); `margin-column` (carries `.jf-signifier-column`) aligns **every** icon in one far-left column (physical-journal left rule). The **horizontal** position of both gutters is set by **measurement, not theme-specific CSS** — a fixed CSS offset collides with themes/snippets that restyle bullets/checkboxes/indentation. Reading view: `gutter-positioner.ts` batches all `getBoundingClientRect` reads then all `left` writes in one `requestAnimationFrame` (one reflow). Editing view: the live-preview `ViewPlugin` uses CodeMirror's `coordsAtPos` inside a batched `view.requestMeasure` on `docChanged/viewportChanged/geometryChanged/placementChanged`. The offset is scroll-invariant (marker + reference move together) so there is **no scroll recompute**; markers are `visibility:hidden` until the first measurement reveals them (`.jf-positioned`, no wrong-x flash). Recompute triggers (both margin modes): after the post-processor, on `css-change` (theme metrics), on `active-leaf-change`/`layout-change`/`file-open` (Obsidian can restore a cached preview without re-running the post-processor), and — crucially — via a **`ResizeObserver` on each `.markdown-preview-sizer`** (observing the *border box*, so our own padding can't loop it) which fires on the sizer-width changes that carry no catchable workspace event (readable-line-width toggle, window resize, sidebar toggle). The settings dropdown describes each option plainly (no warnings — the measured positioning is theme-robust). The `signifierReserveGutter` toggle (global-only, **on by default**, margin modes only) reserves a left lane on the content container's `padding-inline-start` equal to the **deficit** by which the leftmost icon would clip past the pane's nearest overflow-clipping ancestor (NOT the centered sizer) — 0 when the existing margin already fits — so icons never clip with readable line width off / narrow views. The current reserve is read back from the live inline padding (not a `WeakMap`, which desynced on navigate-back) so it can't oscillate, and the base padding is `WeakMap`-cached so it never compounds. Signifiers apply to any rendered markdown, not just tasks, and have their **own settings tab**. Signifiers and task categories are **global-only** (in `GLOBAL_ONLY_FIELDS`, never per-folder or per-flow); task categories live under the Tasks tab. See [docs/signifiers.md](docs/signifiers.md).
